...
{ 
  ...
  //Scintillator (fills the world volume)
  G4double a, z, density, fractionmass;
  G4int nelements; 
  // Get nist material manager
  G4NistManager* nist = G4NistManager::Instance(); 
  // Steel sphere parameters
  //? Buffer and Steel Sphere
  G4double buffer_inner_radius = 65*cm;
  G4double buffer_outer_radius = 115*cm;  
  G4double sphere_inner_radius = 115*cm;  //KamLAND: 650*cm radius, 1m3: 65*cm, 1m3 with buffer: 115*cm
  G4double sphere_outer_radius = 135*cm;  //KamLAND: 660*cm, 1m3: 85*cm, 1m3 with buffer: 135*cm
  ...
  myMPT2->AddProperty("RINDEX",Ephoton_vector.data(),value_vector.data(),Ephoton_vector.size());
  ...
  */
  ...
  myMPT2->AddProperty("ABSLENGTH",Ephoton_vector.data(),value_vector.data(),Ephoton_vector.size()); // ? Buffer
  ...
  //? Buffer
  myMPT2->AddProperty("FASTCOMPONENT",Ephoton_vector.data(), value_vector.data(), Ephoton_vector.size());
  myMPT2->AddProperty("SLOWCOMPONENT",Ephoton_vector.data(), value_vector.data(), Ephoton_vector.size());
  ClearDataVectors();
  ...
  //? Buffer
  myMPT2->AddConstProperty("FASTTIMECONSTANT", 6.9*ns); // KamLAND: 6.9
  myMPT2->AddConstProperty("SLOWTIMECONSTANT",8.8*ns); // KamLAND: 8.8
  //?
  ...
  //? Buffer
  myMPT2->AddConstProperty("FASTSCINTILLATIONRISETIME",1.0*ns); // 1.0ns typical scale
  myMPT2->AddConstProperty("SLOWSCINTILLATIONRISETIME",1.0*ns); // 1.0ns typical scale
  ...
  //? Buffer
  myMPT2->AddConstProperty("YIELDRATIO",0.87);  //KamLAND: 0.87
  ...
  //? Buffer volume, comment out if not simulated
  G4Material* buffer_mat = new G4Material("Buffer-Oil",density=0.7752*g/cm3,nelements=4); //density: 0.8*0.75 + 0.2*0.876  
  buffer_mat->AddElement(C,fractionmass=85.8090*perCent);
  buffer_mat->AddElement(H,fractionmass=14.1644*perCent);
  buffer_mat->AddElement(N,fractionmass=0.0124*perCent);
  buffer_mat->AddElement(O,fractionmass=0.0142*perCent);

  buffer_mat->SetMaterialPropertiesTable(myMPT2);
  //end of Buffer specification
  ...
  // Buffer layer
  // 
  G4Sphere* solidBuffer =
    new G4Sphere("Buffer_Layer",                    //its name
                 buffer_inner_radius,
                 buffer_outer_radius,
                 0.,2*pi,   //phi range
                 0.,pi);    //theta range
  ...
  G4LogicalVolume* logicBuffer =
    new G4LogicalVolume(solidBuffer,            //its solid
                        buffer_mat,             //its material
                        "Buffer_Layer");         //its name

  G4VPhysicalVolume* physicalBuffer = 
    new G4PVPlacement(0,                       //no rotation
                    G4ThreeVector(),         //at (0,0,0)
  ...
  new G4LogicalBorderSurface("WaterSurface",physicalBuffer,physicalSphere,SphereInnerSurface);  
